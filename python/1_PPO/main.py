...
def train_episode(env, agent, update_timestep=256):
    ...
    while True:
        _, reward, done, _ = env.cur_step(agent.act(state))
...
def evaluate(env, agent):
    ...
    while True:
        ...
        state, _, done, info = env.cur_step(action)
    ...
    return {
        ...
        "equal_weight_end": equal[-1],
        "equal_weight_return": equal[-1] / equal[0] - 1,
...
def main():
    ...
    start = dates[0]
    while True:
        train_end = start + np.timedelta64(365 * cfg.TRAIN_DAYS, "D")
        test_end = train_end + np.timedelta64(365 * cfg.TEST_DAYS, "D")
        if test_end > dates[-1]: break
        train_raw = StocksEnv(args.data_dir, start_date=start, end_date=train_end, initial_capital=cfg.initial_capital)
        scaler = StandardScaler().fit(train_raw.features.reshape(len(train_raw.dates), -1))
        train_env = StocksEnv(args.data_dir, start_date=start, end_date=train_end, initial_capital=cfg.initial_capital, scaler=scaler)
        test_env = StocksEnv(args.data_dir, start_date=train_end, end_date=test_end, initial_capital=cfg.initial_capital, scaler=scaler)
        agent = PPOAgent(train_env.state_dim, train_env.action_dim)
        for _ in range(cfg.EPISODES):
            train_episode(train_env, agent, cfg.UPDATE_TIMESTAMP)
        ...
        start = start + np.timedelta64(365 * cfg.STEP_DAYS, "D")
    if not reports:
